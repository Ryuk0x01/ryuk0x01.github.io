...
description: "A detailed write-up for the Intigriti June Bonus Challenge 2026 explaining the reconnaissance process, identifying the vulnerable /share endpoint, and exploiting a CSRF vulnerability to retrieve the secret recipe."
...
> _If the administrator bot could be tricked into visiting an attacker-controlled webpage, and if the `/share` endpoint accepted cross-site POST requests without CSRF protection, the administrator could unknowingly share their private recipe vault with an attacker-controlled account._
